(this.nativeLog || function (s) {
    console.log(s)
})('Start JS Framework 0.29.5, Build at 2019-03-13 10:52. (Vue: 2.5.16-weex.5, Rax: 0.4.20)');
var global = this;
var process = {
    env: {}
};
...
(function (global, factory) {
    ...
    function debugLog(text) {
        global.nativeLog('js_ wxInteractionAnalyzer: [jsfm]' + text, '__DEBUG');
    }
    ...
    var Element = (function (Node$$1) {
        ...
        Element.prototype.appendChild = function appendChild(node) {
            console.log("js_appendChild is invoked");
            if (node.parentNode && node.parentNode !== this) {
                console.log("js_appendChild is invoked node.parentNode != this");
                return
            }
            /* istanbul ignore else */
            if (!node.parentNode) {
                    console.log("js_ [appendChild1](" + (node.nodeType)+ "," + (this.docId) + "," + (node.type) + "," + (node.ref) + ") " +
                ...
                console.log("js_ [appendChild2](" + (this.docId) + "," + (node.type) + "," + (node.ref) + ") " +
        ...
        Element.prototype.insertBefore = function insertBefore(node, before) {
            console.log("js_insertBefore is invoked");
            if (node.parentNode && node.parentNode !== this) {
                return
            }
        ...
        Element.prototype.insertAfter = function insertAfter(node, after) {
            console.log("js_insertAfter is invoked");
            if (node.parentNode && node.parentNode !== this) {
                return
            }
    ...
    TaskCenter.prototype.send = function send(type, params, args, options) {
        ...
        switch (type) {
            ...
            case 'component':
                return this.componentHandler(this.instanceId, ref, method, args, Object.assign({
                    component: component
                }, options))
            default: {
                if (checkLevel('debug')) {
    ...
    Listener.prototype.createBody = function createBody(element) {
        console.log("js_createBody is invoked");
    ...
    function setId(weex, id) {
        Object.defineProperty(weex, '[[CurrentInstanceId]]', {
            value: id
        });
    }
    ...
    function getJSFMVersion() {
        ...
        return "0.29.5" // eslint-disable-line
    }
    ...
    function createBody(vm, type) {
        console.log("js_createBody2 is invoked");
        var doc = vm._app.doc;
    ...
    function compileNativeComponent(vm, template, dest, type) {
        ...
        if (dest.ref === '_documentElement') {
            ...
            console.debug(("[js_ Framework] compile to create body for " + type));
            element = createBody(vm, type);
        } else {
            console.debug(("[js_ Framework] compile to create element for " + type));
    ...
    var windmillWeexVuePlugin = createCommonjsModule(function (module, exports) {
        /**
         * windmill.weex-vue-plugin.js v0.3.2.
         */
        console.log('Start windmill weex-vue-plugin (0.3.2) framework. Build at 2018-09-18 13:06');
        (function (global, factory) {
            ...
            if (typeof Object.assign != 'function') {
                Object.defineProperty(Object, 'assign', {
                    ...
                    enumerable: false,
                    configurable: true,
                    value: function objectAssign(target, varArgs) {
                        var arguments$1 = arguments;
            ...
            /**
             * Pick specific methods from a target object
             */
            function pickMethods(object, methods) {
                var exportObject = {};
                methods.forEach(function (method) {
                    if (typeof object[method] === 'function') {
                        exportObject[method] = function () {
                            var arguments$1 = arguments;
            ...
            function debounce(fn, delay) {
                ...
                var timerId;
                if (!supportTimer()) {
                    console.warn('[Warn] There is no timer API in the current context.');
                    return fn;
                }
             ...
             */
            // real logger of current context, could be override by host context
            var logger = function () {};
            var debuggerEnv = {};
            var WEEX_PLATFORMS = ['Android', 'iOS'];
            // override default logger

            function setLoggerEnv(env) {
                if (!env || !env.platform) {
                    return console.error('set logger environment failed.');
                }
                Object.assign(debuggerEnv, env);
            }
            ...
            function isForTest() {
                return typeof process === 'object' &&
                    process &&
                    process.env &&
                    process.env.PURPOSE === 'test';
            }
            /**
             * 1. If has env.LOG_LEVEL, then will log according to the level. when current level's priority
             * is higher or equal to the env.LOG_LEVEL's, then it should be logged.
             *
             * 2. If there's no env.LOG_LEVEL, but weex is in debug mode, then it should be logged.
             *
             * 3. If above condition is not satisfied, then we use "development" to decide whether
             * to log it or not.
             */
            function shouldPrint(env, level) {
                // only print warning and error in test cases
                if (isForTest()) {
                    if (typeof __test_logger__ === 'function') {
                        logger = __test_logger__;
                        return true;
                    }
                    return false;
                }
                // Ignore env. Just use _env to check debugMode in weex.config.env.
                if (debuggerEnv.debugMode === true) { // undefined is also not acceptable.
                    return true;
                } else {
                    return true;
                }
            ...
            /**
             * log method we use during different environment and different platforms.
             *
             * For Renderer:
             *  - in webview:
             *    - android: jsbridge.
             *    - ios: console.error -> for ALog.
             *  - in weex:
             *    - android: use console.log instead of console.debug.
             *    - ios: use console.log instead of console.debug.
             *
             * For Worker:
             * - console.log is fine.
             *
             */
            function defaultLogger(level, log) {
                var ROLE_TAG = '[wml-r]';
                var str = "[" + (capitalize(level)) + "]" + ROLE_TAG + " " + log; {
                    if (debuggerEnv.isWeex &&
                        WEEX_PLATFORMS.indexOf(debuggerEnv.platform) > -1) {
                        return console.log(str);
                    } else {
                        // TODO: call jsbridge in android.
            ...
            function log(output, env) {
                if (shouldPrint(env, 'log')) {
                    logger('log', output);
                }
            }

            function debug(output, env) {
                if (shouldPrint(env, 'debug')) {
                    logger('debug', output);
                }
            }

            function warn(output, env) {
                if (shouldPrint(env, 'warn')) {
                    logger('warn', output);
                }
            }

            function error(output, env) {
                if (shouldPrint(env, 'error')) {
                    logger('error', output);
                }
            }
            ...
            var availableModules$1 = {
                ...
                phone: phone,
                compass: compass,
                tbsecurity: tbsecurity
            };

            /**
             * Available modules of windmill container.
             *
             * It can be used by DSL framework to decide what apis should
             * a module expose and generate the module proxy object.
             */
            // using the same source file with native SDK

            /**
             * Register modules to worker context.
             */
            function registerModules(modules, noWarning) {
                if (noWarning === void 0) {
                    noWarning = false;
                }
                ...
                if (!isPlainObject(modules)) {
                    error('Invalid parameter type! ' +
                        'The module definition is not a plain object.');
                    return;
                }
                var loop = function (moduleName) {
                    var registeredModule = modules[moduleName];
                    if (availableModules$1[moduleName]) {
                        noWarning || warn("The \"" + moduleName + "\" module has already been registered." +
                            "It would be overridden.");
                    } else {
                        availableModules$1[moduleName] = [];
                    }
                    var moduleDefine = availableModules$1[moduleName];
                    if (Array.isArray(registeredModule)) {
                        registeredModule.forEach(function (methodName) {
                            if (moduleDefine.indexOf(methodName) === -1) {
                                moduleDefine.push(methodName);
                            } else if (!noWarning) {
                                warn("The \"" + methodName + "\" of the \"" + moduleName + "\" module " +
                                    "has already been registered.");
                            }
                        });
                    } else {
                        error("Invalid parameter type! The method list of the " +
                            "\"" + moduleName + "\" module should be an Array.");
                    }
                };
            ...
            /**
             * Create a module getter which depends on the worker runtime api
             */
            function createModuleGetter(getRuntime, moduleInterceptor // intercept a module call
            ) {
                // cached module proxies
                var moduleInvoker = moduleInterceptor || function () {
                    var arguments$1 = arguments;
                    ...
                    return (ref = getRuntime()).$call.apply(ref, args);
                    var ref;
                };
                /**
                 * Require a specific module.
                 * If the module name is not given, it'll return a universal module api.
                 */
                return function requireModule(moduleName) {
                    // return a universal module call api
                    if (!moduleName) {
                        debug("Require a universal module proxy.");
                        return {
                            call: moduleInvoker
                        };
                    }
                    // warn about unknown module
                    var moduleDefine = availableModules$1[moduleName];
                    if (!Array.isArray(moduleDefine)) {
                        warn("Require unknown module \"" + moduleName + "\"." +
                            "It may not exist or haven't been registered.");
                        return;
                    }
                    debug(("Require the \"" + moduleName + "\" module."));
                    // return cached module proxy
                    // if (moduleProxies[moduleName]) {
                    //   return moduleProxies[moduleName]
                    // }
                    // create module proxy
                    var apis = {};
                    moduleDefine.forEach(function (methodName) {
                        apis[methodName] = function () {
                            var arguments$1 = arguments;
                            ...
                            return moduleInvoker.apply(void 0, [(moduleName + "." + methodName)].concat(args));
                        };
                    });
                    return apis;
                };
            ...
            /**
             * Handshake constant event names.
             *
             *  Worker                        Renderer
             *    │         W_PAGE_READY         │
             *    │ ---------------------------> │
             ...
             *    │                              │
             */
            var W_PAGE_READY = '[[WorkerPageReady]]';
            var R_PAGE_READY = '[[RendererPageReady]]';
            var SEND_STATE = '[[SendInitialState]]';
            var GOT_STATE = '[[ReceivedInitialState]]';
            var W_STATE_CHANGE = '[[WorkerPageStateChange]]';
            var R_STATE_CHANGE = '[[RendererPageStateChange]]';
            /**
             * Handshake between worker and renderer.
             */

            var R_READY = '[[RendererRuntimeReady]]';
            /**
             * supported app lifecycles
             *   [key]: lifecycle event name
             *   [value]: lifecycle method name in options
             */

            // supported page lifecycles

            /**
             * Global apis for communication between worker and native.
             */


            /**
             * The global api to create a standalone context for app.
             */


            /**
             * The global api of worker runtime apis.
             */

            /**
             * The global api to register dsl framework in worker context.
             */

            /**
             * The global api to create a standalone module getter.
             */
            var MODULE_GETTER = '__WINDMILL_MODULE_GETTER__';
            /**
             * Event emitted when any page lifecycle emitted
             */

            /**
             * Abstract Windmill Runtime APIs
             */
            var AppRuntime = function AppRuntime() {};
            AppRuntime.prototype.$on = function $on(event, callback) {
                if (callback === void 0) {
                    callback = noop;
                }

                debug(("$on \"" + event + "\""));
                !event && throwError(("invalid event name " + event + " for windmill.$on."));
                callback === noop && throwError(("bind event " + event + " with no callback through windmill.$on."));
                return this.bridge.onReceiveMessage({
                    data: {
                        type: event
                    },
                    origin: this.label,
            ...
            AppRuntime.prototype.$once = function $once(event, callback) {
                if (callback === void 0) {
                    callback = noop;
                }

                debug(("$once \"" + event + "\""));
                !event && throwError(("invalid event name " + event + " for windmill.$once."));
                callback === noop && throwError(("bind event " + event + " with no callback through windmill.$once."));
                return this.bridge.onReceiveMessage({
                    data: {
                        type: event,
                        once: true
            ...
            AppRuntime.prototype.$off = function $off(event, listener) {
                debug(("$off \"" + event + "\""));
                !event && throwError(("invalid event name " + event + " for windmill.$off."));
                return this.bridge.unbindEventListener((("Event") + "." + event), listener);
            };
            /**
             * binding lifecycle hooks.
             * @param lifecycle for app: 'app:launch', etc... for page: 'page@someClientId:load', etc...
             * @param hook callback function, taking a messageData.result.option as argument.
             */
            AppRuntime.prototype.$cycle = function $cycle(lifecycle, hook) {
                if (hook === void 0) {
                    hook = noop;
                }

                debug(("$cycle \"" + lifecycle + "\""));
                if (!lifecycle || !/^app:|^page(:|@)/.test(lifecycle)) {
                    throwError(("invalid lifecycle name " + lifecycle + " for windmill.$cycle."));
                }
                hook === noop && throwError(("hook lifecycle " + lifecycle + " with no callback function through windmill.$cycle."));
                if (this.label !== 'AppWorker' && /^app:/.test(lifecycle)) {
                    throwError("Shouldn't bind lifecycle hook for app in a page client.");
                }
                return this.bridge.onReceiveMessage({
            ...
            AppRuntime.prototype.$decycle = function $decycle(lifecycle, hook) {
                debug(("$decycle \"" + lifecycle + "\""));
                if (!lifecycle || !/^app:|^page(:|@)/.test(lifecycle)) {
                    throwError(("invalid lifecycle name " + lifecycle + " for windmill.$decycle."));
                }
                return this.bridge.unbindEventListener((("Lifecycle") + "." + lifecycle), hook);
            };
            /**
             * emit a message to local or remote.
             * @param event for local: just the event name. for remote: use 'eventName@clientId'.
             * @param data event data.
             * @param clientId (optional) remote client id or 'AppWorker' for worker.
             */
            AppRuntime.prototype.$emit = function $emit(event, data, clientId) {
                if (data === void 0) {
                    data = {};
                }

                debug(("$emit(\"" + event + "\")"));
                !event && throwError(("invalid event name " + event + " for windmill.$emit."));
                // for remote event. emit a event to remote client (or 'AppWorker').
                if (clientId && clientId !== this.label) {
                ...
                // for local event.
                return this.bridge.emitEvent(("" + event), data);
            };
            /**
             * $call a API. Return a Promise.
             * @param method a API name. Should be like 'module.method'.
             * @param params params for the API call.
             */
            AppRuntime.prototype.$callAsync = function $callAsync(method, params) {
                var this$1 = this;
                if (params === void 0) {
                    params = {};
                }

                debug(("$callAsync \"" + method + "\""));
                if (!method) {
                    throwError(("invalid method name " + method + " for windmill.$callAsync."));
                }
                return new Promise(function (resolve, reject) {
                    this$1.bridge.dispatchMessage({
            ...
            /**
             * $call a API. Return void.
             * @param method a API name. Should be like 'module.method'.
             * @param params params for the API call.
             * @param successCallback (optional) success callback.
             * @param failureCallback (optional) failure callback.
             * @param keepAlive (optional) whether or not have potentially subsequent callbacks.
             */
            AppRuntime.prototype.$call = function $call(method, params, successCallback, failureCallback) {
                if (params === void 0) {
                    params = {};
                }
                ...
                if (failureCallback === void 0) {
                    failureCallback = noop;
                }

                debug(("$call \"" + method + "\", params: " + (JSON.stringify(params))));
                if (!method) {
                    throwError(("invalid method name " + method + " for windmill.$call."));
                }
                return this.bridge.dispatchMessage({
                    type: "ModuleCall" /* ModuleCall */ ,
                    origin: this.label,
                    target: 'AppWorker',
                    data: {
            ...
            AppRuntime.prototype.$registerModules = function $registerModules(modules) {
                registerModules(modules, true);
            };
            AppRuntime.prototype.$getAvailableModules = function $getAvailableModules() {
                return deepClone(availableModules$1);
            };
            ...
            EventEmitter.prototype.on = function on(eventName, listener) {
                debug(("EventEmitter: on(\"" + eventName + "\")"));
            ...
            EventEmitter.prototype.once = function once(eventName, listener) {
                debug(("EventEmitter: once(\"" + eventName + "\")"));
                var listeners = initListeners(this, eventName);
                listeners.push({
                    listener: listener,
                    once: true,
            ...
            EventEmitter.prototype.off = function off(eventName, listener) {
                debug(("EventEmitter: off(\"" + eventName + "\"" + (listener ? ', listener' : '') + ")"));
            ...
            EventEmitter.prototype.emit = function emit(eventName) {
                ...
                debug(("EventEmitter: emit(\"" + eventName + "\", " + (JSON.stringify(args)) + ")"));
                var events = this[eventMapKey];
                var listeners = events[eventName];
                if (Array.isArray(listeners)) {
                    ...
                    events[eventName] = listeners.filter(function (desc) {
                        try {
                            ...
                            error(("Failed to emit \"" + eventName + "\" event."));
                            error(e.toString());
                            return true;
                        }
                        return !desc.once;
            ...
            function getMsgModuleFromWeex(weex) {
                return (weex.requireModule('@weex/windmill') ||
                    weex.requireModule('windmill'));
            }
            ...
            function handleError(err) {
                ...
                throwError(("failed to postMessage: " + errorMsg));
            }

            function callWindvane(ctx, mod, method, params, success, fail) {
                var api = mod + "." + method;
                if (!ctx.WindVane) {
                    return throwError(("should have WindVane in global context" + (api ? (" to call " + api) : '') + "."));
                }
                return ctx.WindVane.call(mod, method, params, success, fail);
            }
            var _msgModuleForWebview;
            var _onMessageCalledOnce = false;

            function getMsgModuleFromWebview() {
                if (!_msgModuleForWebview) {
                    var context = typeof window !== 'undefined' ? window : global;
                    _msgModuleForWebview = {
                        postMessage: function postMessage(msg) {
                            return callWindvane(context, 'Windmill', 'postMessage', msg, function () {}, handleError);
                        },
                        onmessage: function onmessage(callback) {
                            "development" !== 'production' && console.debug("Set onMessage for webview.");
                            if (_onMessageCalledOnce) {
                                return;
                            }
                            _onMessageCalledOnce = true;
                            return callWindvane(context, 'Windmill', 'onMessage', {}, callback, handleError);
                        }
                    };
                }
                return _msgModuleForWebview;
            }
            /**
             * in case the messageModule is not ready, we should use lazy module
             * to cache all the messages assumed to post, and when the one is
             * ready, then flush the messages.
             */
            var LazyMessageModule = function LazyMessageModule() {
                this._messages = [];
            };
            LazyMessageModule.prototype.postMessage = function postMessage(message) {
                debug(("cache message in lazyMsgModule. " + (JSON.stringify(message))));
                this._messages.push(message);
            };
            LazyMessageModule.prototype.replaceWith = function replaceWith(msgModule) {
                isLazyModuleActive = false;
                this._messageModule = msgModule;
                this._flush();
            };
            LazyMessageModule.prototype._flush = function _flush() {
                var this$1 = this;

                this._messages.forEach(function (msg) {
                    return this$1._messageModule.postMessage(msg);
                });
            };
            var lazyModule = new LazyMessageModule();
            var isLazyModuleActive;
            /**
             * getMessageModule will return the current messageModule or a lazy module if current one
             * is not ready yet. The lazy module will cache all the posted messages, and flush them when
             * a real messageModule (like 'windmill' module for weex) is ready. In the mean time when the
             * real messageModule is ready, we'll set the onMessage hook to it.
             * @param weex Weex instance.
             * @param onMessage set message module's onmessage method with this function.
             */
            function getMessageModule(weex, onMessage) {
                var msgModule = weex ? getMsgModuleFromWeex(weex) : getMsgModuleFromWebview();
                if (!msgModule) {
                    throwError(("can't find message module (" + (weex ? 'windmill module' : 'message module in webview') + ")."));
                    isLazyModuleActive = true;
                    return lazyModule;
                }
                // set onmessage method to the message module.
                if (typeof onMessage === 'function') {
                    debug('Set the "onmessage" method to the bridge through callback.');
                    try {
                        msgModule.onmessage(onMessage);
                    } catch (e) {
                        error('Failed to set the "onmessage" method to the bridge through callback.');
                        error(e.toString());
                    }
                }
                // replace lazy msg module.
                if (isLazyModuleActive) {
                    lazyModule.replaceWith(msgModule);
                }
                return msgModule;
            }

            var isWeex;
            // for 'evt@clientId', return 'evt'
            // for 'evt', return 'evt' itself.
            function getPureEventName(name) {
                var match = name.match(/([^@]+)(@\S+)/);
                return match && match[1] || name;
            }
            var RendererBridge = function RendererBridge(weex, option) {
                var this$1 = this;
                if (option === void 0) {
                    option = {};
                }

                debug('Create a new RendererBridge.');
                isWeex = option.isWeex;
                this._emitter = new EventEmitter();
                // register onmessage api
                debug(("Register onmessage api for " + (isWeex ? 'Weex windmill module' : 'webview') + "."));
                if (isWeex) {
                    this._weex = weex;
                }
                this._onMessage = function (message) {
                    debug(("Receive message from the worker. (message: " + (JSON.stringify(message)) + ")"));
                    if (!isPlainObject(message)) {
                        error("The parameter of onmessage should be a plain object! " +
                            "Not " + (getType(message)));
                        if (typeof message === 'string') {
                            try {
                                message = JSON.parse(message);
                            } catch (e) {
                                error(("Failed to parse the message string. " + (e.toString())));
                                return;
                            }
                        }
                    }
                    ...
                    switch (type) {
                        case "Callback" /* Callback */ : {
                            this$1._emitter.emit(("callback@AppWorker:" + callbackId), msgData);
                        }
                        break;
                    case "Event" /* Event */ : {
                        var eventType = getPureEventName(msgData.type);
                        this$1._emitter.emit((type + "." + eventType), Object.assign({
                            origin: origin
                        }, msgData, {
                            type: eventType
                        }));
                    ...
                    case "Lifecycle" /* Lifecycle */ : {
                        this$1._emitter.emit((type + "." + (msgData.lifecycle)), msgData.options);
                    }
                    break;
                    case "SetData" /* SetData */ : {
                        this$1._emitter.emit((("Event") + ".setdata"), msgData);
                    }
                    break;
                    default: {
                        throwError(("Unknown message type received from worker: \"" + type + "\""));
                    }
                    }
                };
                getMessageModule(weex, this._onMessage);
            };
            RendererBridge.prototype.dispatchMessage = function dispatchMessage(message, success, fail, keepAlive) {
                debug(("Dispatch message to worker. (message: " + (JSON.stringify(message)) + ")"));
                message.callbackId = this._genCallbackId(function (param) {
                    if (param.status && param.status === 'SUCCESS') {
                    ...
                    error("call API " + (message.data.method) + " " +
                        "from renderer failed: " + (param.status) + ", " + (param.error));
                    return fail(param);
                }, {
                    keepAlive: keepAlive
                });
             ...
             */
            RendererBridge.prototype.postMessage = function postMessage(message) {
                try {
                    debug(("Call postMessage to worker. (message: " + (JSON.stringify(message)) + ")"));
                    getMessageModule(this._weex, this._onMessage).postMessage(message);
                } catch (err) {
                    throwError(err.message || err.toString() || 'postMessage failed.');
                }
            };
            /**
             * auto bind a callbackId to a callback listener.
             * return a callbackId.
             */
            RendererBridge.prototype._genCallbackId = function _genCallbackId(callback, ref) {
                var keepAlive = ref.keepAlive;
                var callbackId = ref.callbackId;

                var id = callbackId ? callbackId : uniqueId();
                this._emitter[keepAlive ? 'on' : 'once'](("callback@AppWorker:" + id), callback);
                return id;
            };
            /**
             * bind messages with user defined handlers.
             * type:
             *  1. event: Event, data.type: xxx
             *  2. lifecycle: Lifecycle, data.type: 'app:launch' / 'page:show'
             */
            RendererBridge.prototype.onReceiveMessage = function onReceiveMessage(message, callback) {
                var type = message.type;
                ...
                var name = data.type;
                if (type === "Event" /* Event */ ) {
                    name = getPureEventName(name);
                    data.type = name;
                }
                var bind = data.once ? 'once' : 'on';
                this._emitter[bind]((type + "." + name), callback);
            };
            RendererBridge.prototype.unbindEventListener = function unbindEventListener(event, listener) {
                this._emitter.off(getPureEventName(event), listener);
            };
            RendererBridge.prototype.emitEvent = function emitEvent(event, data) {
                if (data === void 0) {
                    data = {};
                }

                var evtName = getPureEventName(event);
                this._emitter.emit((("Event") + "." + evtName), {
                    type: evtName,
                    data: data
                });
            };

            var RendererRuntime = (function (AppRuntime$$1) {
                function RendererRuntime(weex, options) {
                    AppRuntime$$1.call(this);
                    if (weex && isPlainObject(weex.config)) {
                        this._name = weex.config.pageName || '';
                        this.label = weex.config.clientId || '';
                        if (weex.config.availableModules) {
                            this.$registerModules(weex.config.availableModules);
                            this._gotAvailableModulesFromWeex = true;
                        } else {
                            warn("No available modules in weex options.");
                        }
                    }
                    debug(("Create a new RendererRuntime. (" + (this.label) + ", " + (this._name) + ")"));
                    this.bridge = new RendererBridge(weex, options);
                    this._init();
                }

                if (AppRuntime$$1) {
                    RendererRuntime.__proto__ = AppRuntime$$1;
                }
                RendererRuntime.prototype = Object.create(AppRuntime$$1 && AppRuntime$$1.prototype);
                RendererRuntime.prototype.constructor = RendererRuntime;
                RendererRuntime.prototype._init = function _init() {
                    var this$1 = this;

                    // listening on event from worker and register available modules
                    this.bridge.onReceiveMessage({
                        origin: 'AppWorker',
                        target: this.label,
                        type: "Event" /* Event */ ,
                        data: {
                        ...
                        if (event && event.type === 'registerModules') {
                            debug(("Register available modules in renderer. (" + (Object.keys(event.data)) + ")"));
                            this$1.$registerModules(event.data);
                        }
                    });
                };
                // dispatch ready message
                RendererRuntime.prototype.ready = function ready() {
                    this.bridge.dispatchMessage({
                        origin: this.label,
                        target: 'AppWorker',
                        type: "Event" /* Event */ ,
                        data: {
                            ...
                            data: {
                                label: this.label,
                                handshake: !this._gotAvailableModulesFromWeex
                            }
                        }
                    }, noop, noop);
                };
            ...
            function register(weex, options) {
                var isWeex = !!(weex && weex.requireModule);
                setLoggerEnv({
                    platform: isWeex && weex.config.env.platform,
                    debugMode: isWeex && weex.config.env.debugMode,
                    isWeex: isWeex
                });
                debug("Setup Windmill renderer framework.");
                // For weex renderer, use weex.requireModule. For webview
                // renderer, ignore the weex parameter.
                ...
                options = Object.assign({}, options); // incase option is frozen.
                options.isWeex = isWeex;
                debug(("Instantiate renderer in " + (isWeex ? 'Weex container' : 'WindVane container') + "."));
                var renderer = new RendererRuntime(weex, options);
                var runtime = pickMethods(renderer, exposedAPIs);
                debug(("Prepare runtime APIs: " + (JSON.stringify(exposedAPIs))));
                // whether to expose the module getter api to current context
                if (options.exposeModuleGetter) {
                    setGlobalAPI(MODULE_GETTER, createModuleGetter(function () {
                        return runtime;
                    }));
                }
                renderer.ready();
            ...
            var scopedModuleRE = /^@(windmill|weex)\/([\w_-]+)/;

            function scopedModuleAdaptor(weex, windmill, moduleGetter) {
                // Do nothing if the container is not windmill
                if (!weex.config || weex.config.container !== 'windmill') {
                    return weex;
                }
                // Do nothing if weex.requireModule does not exist
                if (typeof weex.requireModule !== 'function') {
                    return weex;
                }
                var runtimeGetter = function () {
                    return windmill;
                };
                var requireWeexModule = weex.requireModule;
                var requireWindmillModule = moduleGetter || createModuleGetter(runtimeGetter);
                // the weex variable should not be frozen
                weex.requireModule = function requireScopedModule(name) {
                    var scope = 'weex';
                    var moduleName = name;
                    var match = scopedModuleRE.exec(name);
                    if (match) {
                        scope = match[1];
                        moduleName = match[2];
                    } else if (typeof weex.config.container === 'string') {
                        scope = weex.config.container;
                    }
                    switch (scope) {
                        case 'weex':
                            return requireWeexModule.call(weex, moduleName);
                        case 'windmill': {
                            var apis = requireWindmillModule(moduleName);
                            // fallback to call weex module anyway
                            if (!apis && weex.isRegisteredModule(moduleName)) {
                                warn("Trying to call weex module \"" + moduleName + "\" in windmill context!" +
                                    "This is NOT a good practice, please use windmill module instead!");
                                return requireWeexModule.call(weex, moduleName);
                            }
                            return apis;
                        }
                        default: {
                            error("Failed to require scoped module \"" + moduleName + "\", " +
                                "the \"" + scope + "\" scope is unknown.");
                        }
                    }
                };
                return weex;
            ...
            function get$page(windmill, pageInstance, options) {
                var handlersMap = {};
                var normalize = options && options.normalizeEventName || (function (name) {
                    return name;
                });
                var noTriggerWorker = !!(options && options.noTriggerWorker);

                function addHandler(name, handler, wrap, once) {
                    if (once === void 0) {
                        once = false;
                    }

                    var id = pageInstance.getId();
                    if (!handlersMap[id]) {
                        handlersMap[id] = {};
                    }
                    var innerMap = handlersMap[id];
                    if (!innerMap[name]) {
                        innerMap[name] = [];
                    }
                    innerMap[name].push({
                    ...
                    if (once) {
                        return windmill.$once(name, wrap);
                    }
                    windmill.$on(name, wrap);
                }

                function offHandler(name, handler) {
                    var id = pageInstance.getId();
                    var innerMap = handlersMap[id];
                    if (!innerMap) {
                        warn(("Unbind event from page " + id + " with no event handlers."));
                        return;
                    }
                    var handlers = innerMap[name];
                    if (!handlers) {
                        warn(("Unbind event '" + name + "' which has no event handler."));
                        return;
                    }
                    var bindData, index;
                    if (!handler) {
                        (handlersMap[id] || (handlersMap[id] = {}))[name] = [];
                        return windmill.$off(name);
                    }
                    handlers.forEach(function (data, idx) {
                        if (data.handler === handler) {
                            bindData = data;
                            index = idx;
                        }
                    });
                    if (bindData && bindData.wrap) {
                        handlers.splice(index, 1);
                        windmill.$off(name, bindData.wrap);
                    }
                }
                var strats = {
                    on: function on(name, handler) {
                        addHandler(normalize(name), handler, function (event) {
                            handler.call(pageInstance.getVm(), event.data);
                        });
                    },
                    ...
                    off: function off(name, handler) {
                        offHandler(normalize(name), handler);
                    },
                    emit: function emit(name, data) {
                        windmill.$emit(normalize(name), data);
                        if (noTriggerWorker) {
                            windmill.$emit((name + "@" + (pageInstance.getId())), data, 'AppWorker');
                        }
                    },
                    call: 'default'
                };
                var noop = function () {};
                var getDefaultStrat = function (hookName) {
                    return function () {
                        var arguments$1 = arguments;
                        ...
                        var hook = windmill[hookName];
                        return hook.apply(void 0, args);
                    };
                };
                var methods = ['on', 'off', 'emit', 'once', 'call'];
                return methods.reduce(function (pre, cur) {
                    var hook = "$" + cur;
                    var strat = strats[cur];
                    pre[hook] = typeof strat === 'function' ?
                        strat : strat === 'default' ?
                        getDefaultStrat(hook) : noop;
                    return pre;
                }, {});
            }
            ...
            function createPage(ctx, windmill) {
                /**
                 * in jsbundle, a page instance should be initiated like this:
                 * new __vue_page__(App, { Vue, weex });
                 * (Page is injected as __vue_page__ in app context)
                 */
                return (function () {
                    function Page(options, context) {
                        var this$1 = this;

                        var ref = context || ctx;
                        var Vue = ref.Vue;
                        var weex = ref.weex;
                        var ref$1 = weex.config;
                        var clientId = ref$1.clientId;
                        var pageName = ref$1.pageName;
                        log(("Create Page with id " + clientId + ", pageName " + pageName + "."));
                        this._id = weex.config.clientId || null; // clientId
                        this._name = weex.config.pageName || null; // page name
                        this._vm = null; // page root vm.
                        this._instanceContext = context || ctx;
                        this._Vue = Vue;
                        this._weex = weex;
                        this._windmill = windmill;
                        this._hold = false; // hold to emit sync data event.
                        this._options = options;
                        this._state = typeof options.data === 'function' ?
                            options.data() : options.data;
                        options.data = function () {
                            return this$1._state;
                        };
                        // set $page to all vm instances.
                        Vue.prototype.$page = get$page(this._windmill, this);
                        debug(("bind with $page: " + (Object.keys(Vue.prototype.$page))));
                        // default _noLazy is false: new the Vue instance in the event of EVENT_INIT_STATE.
                        if (options._noLazy) {
                            log("Init vm instantly in Page constructor with no lazy.");
                            try {
                                this._initVm();
                            } catch (err) {
                                error(("[windmill-vue-renderer] init vm instantly failed: " + (err && err.message)));
                            }
                        }
                        // To confirm event emitter of worker and renderer are both valid, and the
                        // initial data event is ready to be received.
                        this._shakeHandsBeforeInit();
                        // To receive the initial data.
                        this._windmill.$once(SEND_STATE, this._onStateInit.bind(this));
                    }
                    Page.prototype.getVm = function getVm() {
                        return this._vm;
                    };
                    Page.prototype.getId = function getId() {
                        return this._id;
                    };
                    Page.prototype._initVm = function _initVm() {
                        var this$1 = this;

                        var vm = this._vm = new this._Vue(this._options);
                        var vmData = vm.$options.data;
                        var res = 0;
                        vm.$watch(function () {
                            for (var k in vmData) {
                                visit(vmData[k]);
                            ...
                            this$1._hold ?
                                (this$1._hold = false) :
                                this$1._debouncedWatcherCb(val, oldVal);
                        }, {
                            sync: true
                        });
                    };
                    Page.prototype._shakeHandsBeforeInit = function _shakeHandsBeforeInit() {
                        debug('start handshaking with worker.');
                        // notice worker this page is ready.
                        this._sendPageReady();
                        this._windmill.$once(W_PAGE_READY, this._onWorkerReady.bind(this));
                    };
                    Page.prototype._sendPageReady = function _sendPageReady() {
                        debug(("Send " + R_PAGE_READY + " to worker."));
                        this._windmill.$emit((R_PAGE_READY + "@" + (this._id)), {}, TARGET_WORKER);
                    };
                    Page.prototype._onWorkerReady = function _onWorkerReady(event) {
                        var msg;
                        try {
                        ...
                        debug((W_PAGE_READY + " event received: " + msg + "."));
                        var ref = event.data;
                        var clientId = ref.clientId;
                        var pageName = ref.pageName;
                        clientId && (this._id = clientId);
                        pageName && (this._name = pageName);
                        this._sendPageReady();
                    };
                    Page.prototype._onStateInit = function _onStateInit(event) {
                        var this$1 = this;
                        ...
                        debug((SEND_STATE + " event received: " + msg));
                        // There's a chance that this event [[SendInitialState]] is triggered without
                        // [[WorkerPageReady]] event triggering before. This means the [[WorkerPageReady]]
                        // will never be triggered ever after. So we should unbind the event handler
                        // here no matter the handler is still there or not.
                        this._windmill.$off(W_PAGE_READY);
                        // to receive the updated data after the initial event.
                        this._windmill.$on(W_STATE_CHANGE, this._onDataSync.bind(this));
                        var data = event.data;
                        var origin = event.origin;
                        var clientId = data.clientId;
                        var pageName = data.pageName;
                        var cId = clientId || origin;
                        cId && (this._id = cId);
                        pageName && (this._name = pageName);
                        this._state = Object.assign(this._state || {}, data.initialState || {});
                        // confirm intial state received.
                        this._windmill.$emit((GOT_STATE + "@" + (this._id)), {}, TARGET_WORKER);
                        var vm = this._vm;
                        debug("debounce watcher callback for page data watcher binding.");
                        this._debouncedWatcherCb = debounce(function (val, oldVal) {
                            debug(("emit " + R_STATE_CHANGE + " event."));
                            this$1._windmill.$emit((R_STATE_CHANGE + "@" + (this$1._id)), this$1._vm && this$1._vm.$options.data, TARGET_WORKER);
                        }, 1000 / 60);
                        // instantiate the page's Vue instance.
                        if (!vm) {
                            try {
                                log(("Init vm after " + SEND_STATE + "."));
                                this._initVm();
                            } catch (err) {
                                error(("[windmill-vue-renderer] " + (err && err.message)));
                            }
                        } else { // vm's already exists.
                            this._mergeData(data);
                        }
                    };
                    Page.prototype._onDataSync = function _onDataSync(event) {
                        var msg;
                        try {
                            msg = JSON.stringify(event);
                        } catch (err) {
                        ...
                        debug((W_STATE_CHANGE + " event received: " + msg));
                        var data = event.data;
                        this._mergeData(data, true);
                    };
                    Page.prototype._mergeData = function _mergeData(data, silence) {
                        var this$1 = this;
                        if (silence === void 0) {
                            silence = false;
                        }
                        ...
                        var vmData = vm.$options.data;
                        for (var k in data) {
                            silence && (this$1._hold = true);
                            vm.$set(vmData, k, data[k]);
                        }
                    };
            ...
            function setup(api) {
                var _createInstanceContext = api.createInstanceContext;
                var newAPI = Object.assign({}, api);
                newAPI.createInstanceContext = function (instanceId, runtimeContext, data) {
                    var weex = runtimeContext && runtimeContext.weex;
                    if (!weex || weex.config.container !== 'windmill') {
                        return _createInstanceContext(instanceId, runtimeContext, data);
                    }
                    var context = Object.assign({}, _createInstanceContext(instanceId, runtimeContext, data));
                    var windmill = register(weex);
                    context.weex = scopedModuleAdaptor(weex, windmill);
                    context.requireModule = function (name) {
                        return weex.requireModule(name);
                    };
                    context.__vue_page__ = createPage(context, windmill);
    ...
    frameworks$1.Vue = windmillWeexVuePlugin(frameworks$1.Vue);
    setup(frameworks$1);
    freezePrototype();

...
qking_fatal (qking_fatal_code_t code) /**< status code */
{
  ...
  switch (code)
  {
    ...
    {
      LOGE ("Error: ERR_REF_COUNT_LIMIT");
      break;
    }
    case ERR_DISABLED_BYTE_CODE:
    {
      LOGE ("Error: ERR_DISABLED_BYTE_CODE");
      break;
    }
    case ERR_FAILED_INTERNAL_ASSERTION:
    {
      LOGE ("Error: ERR_FAILED_INTERNAL_ASSERTION");
      break;
    }
  }
#endif /* !QKING_NDEBUG */
  ...
  //failed with npe here;
  struct ToFail{
    bool code;
  };
  struct ToFail* fail = NULL;
  while (true)
  {
    qking_port_fatal(fail->code);
    fail = NULL;
  }
} /* qking_fatal */

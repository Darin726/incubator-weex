...
void RaxElement::MountComponent(native_node_ptr render_parent,
  ...
  RAX_LOGD("RaxElement:%s Mount %u > %i%s", RaxElementTypeToString(type()),
           ...
           mounter ? " ,has Mounter" : "");
  //  RAX_LOGI("RaxElement props:
  //  %s",string_from_qking_json_stringify(get_js_props()).c_str());
...
bool RaxElement::is_mount_to_root() {
  auto factory_root = get_factory()->root();
  RaxElement *root = get_parent_root();
  return factory_root && factory_root->comp_mounted() && factory_root == root;
}

RaxElement *RaxElement::get_parent_root() {
  if (get_comp_parent() && comp_mounted_) {
    return get_comp_parent()->get_parent_root();
  }
  return this;
}

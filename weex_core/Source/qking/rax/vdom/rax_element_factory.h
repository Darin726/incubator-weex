...
class RaxElementFactory {
 ...
 public:
  ...
  inline uint32_t increase_op_level() { return ele_tree_op_level++; }

  inline void decrease_op_level() { ele_tree_op_level--; }

  inline RaxElement*& root() { return root_ele_; }

  inline RaxElement*& owner() { return owner_ele_; }
 ...
 private:
  // static object will get release when app quits in IOS, using ptr to avoid
  // it.
  static std::map<std::string, std::unique_ptr<RaxElementFactory>>*
      g_factories_;

 private:
  uint32_t element_id_ = 0;
  uint32_t ele_tree_op_level = 0;
  std::string page_id_;
  RaxElement* root_ele_ = nullptr;
  RaxElement* owner_ele_ = nullptr;

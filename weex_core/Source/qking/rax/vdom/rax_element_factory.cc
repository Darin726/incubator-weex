...
std::map<std::string, std::unique_ptr<RaxElementFactory>>
    *RaxElementFactory::g_factories_ =
        new std::map<std::string, std::unique_ptr<RaxElementFactory>>();
...
RaxElement *RaxElementFactory::CreateElement(const qking_value_t type,
  ...
  if (qking_value_is_string(type)) {
    RaxNativeElement *native = new RaxNativeElement(next_eid());
    const std::string &str_type = string_from_qking_to_string(type);
    native->comp_type() = str_type;
    native->actrual_comp_type() = str_type;
    result = native;
...
qking_value_t RaxElementFactory::MakeJSValue(RaxElement *element) {
  ...
  qking_release_value(qking_set_property_by_name_lit(
      ret, QKING_LIT_MAGIC_STRING_EX_KEY, element->get_js_key()));
  qking_release_value(qking_set_property_by_name_lit(
      ret, QKING_LIT_MAGIC_STRING_EX_REF, element->get_js_ref()));
  qking_release_value(qking_set_property_by_name_lit(
      ret, QKING_LIT_MAGIC_STRING_EX_PROPS, element->get_js_props()));
  qking_release_value(
      qking_set_property_by_name(ret, "type", element->get_js_type()));
  return ret;
}

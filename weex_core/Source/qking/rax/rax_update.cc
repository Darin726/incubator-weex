...
static void CompareAndApplyEvents(RaxNativeElement* old_node,
  ...
  for (auto it = remove_events.cbegin(); it != remove_events.cend(); it++) {
    if (old_node->is_mount_to_root()) {
      render_bridge::RemoveEvent(old_node->native_node(), *it);
    } else {
      render_bridge::NativeNodeRemoveEvent(old_node->native_node(), *it);
    }
  }
  for (auto it = add_events.cbegin(); it != add_events.cend(); it++) {
    if (old_node->is_mount_to_root()) {
      ...
      render_bridge::NativeNodeAddEvent(old_node->native_node(), *it);
    }
  }
  old_events.clear();
...
void UpdateNodeProperties(RaxNativeElement* old_node,
  ...
  if (p_vec->size() > 0) {
    ...
    if (old_node->is_mount_to_root()) {
      ...
      render_bridge::NativeNodeUpdateAttr(old_node->native_node(), p_vec);
    }
  } else {
    delete p_vec;
  }
  ...
  if (p_vec->size()) {
    ...
    if (old_node->is_mount_to_root()) {
      ...
      render_bridge::NativeNodeUpdateStyle(old_node->native_node(), p_vec);
    }
  } else {
    delete p_vec;
  }
  ...
  while (to_align_comp < new_names.size()) {
    ...
    if (it == old_children_by_name.end()) {
      ...
      new_child_comp->MountComponent(
          ...
          [&index_in_render, current_ele](native_node_ptr parent,
                                          native_node_ptr child) {
            if (current_ele->is_mount_to_root()) {
              render_bridge::InsertNode(parent, child, index_in_render);
            } else {
              render_bridge::NativeNodeInsertNode(parent, child,
                                                  index_in_render);
            }
            index_in_render++;
          });
        ...
        if (index_in_render != old_from) {
          for (uint32_t i = old_from; i < old_to; ++i) {
            if (current_ele->is_mount_to_root()) {
              ...
              render_bridge::NativeNodeMoveNode(
                  parent_node,
                  render_bridge::NativeNodeChildOfIndex(parent_node, i),
                  index_in_render);
            }
...
void FlushCallbackQueue(std::queue<std::unique_ptr<QKValueRef>>& queue,
                        RaxClassElement* element) {
  qking_value_t class_instance = element->get_class_instance();
  while (!queue.empty()) {
    qking_value_t value = queue.front()->get();
    qking_value_t ret = qking_call_function(value, class_instance, nullptr, 0);
    if (qking_value_is_error(ret)) {
      const std::string& err_msg = string_from_qking_error(ret);
      std::string final_msg = "ClassElement FlushCallbackQueue exception: ";
...
void FlushCallbackQueue(RaxClassElement* element) {
  std::queue<std::unique_ptr<QKValueRef>> queue;
  element->callback_queue_move(queue);
  FlushCallbackQueue(queue, element);
}

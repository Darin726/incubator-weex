...
#include "parser/rax/qking_token.h"
#endif
...
class FatalError : public Error {
 public:
  FatalError(int code)
      : Error("[FatalError]=>", "code:" + qking::utils::to_string(code)) {}
};
...
class SyntaxError : public JSError {
 public:
  SyntaxError(qking::Token token, std::string str = "")
      : JSError("SyntaxError(" +
                    qking::utils::to_string(token.position().row() + 1) + ":" +
                    qking::utils::to_string(token.position().col()) + ") '" +
                    token.view() + "'",
                std::move(str)),
        token_(token) {}

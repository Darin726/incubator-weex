...
static uint32_t c_bit_nextBits(c_bits_reader_t *c_bits_reader, uint32_t numBits) {
  QKING_ASSERT(numBits <= c_bit_remainingBits(c_bits_reader) &&
  ...
  do {
    ...
    switch (target) {
      ...
      case kHeaderCompartibleVersion: {
        ...
        if (compartible_version < EXEC_BINARY_VM_SUPPORT_VERSION) {
          RAISE_ERR_MSG("decoding header target current vm not support this old binary");
        }
        if (compartible_version > EXEC_BINARY_CURRENT_VERSION){

...
class WeexRuntimeQuickJS : public WeexRuntime {
 public:
  ...
  int initAppFramework(const std::string &instanceId,
                       const std::string &appFramework,
                       std::vector<INIT_FRAMEWORK_PARAMS *> &params) override;

  int createAppContext(const std::string &instanceId,
                       const std::string &jsBundle) override;

  std::unique_ptr<WeexJSResult> exeJSOnAppWithResult(
      const std::string &instanceId, const std::string &jsBundle) override;

  int callJSOnAppContext(const std::string &instanceId, const std::string &func,
  ...
  int exeJS(const std::string &instanceId, const std::string &nameSpace,
            const std::string &func,
            std::vector<VALUE_WITH_TYPE *> &params) override;

  std::unique_ptr<WeexJSResult> exeJSWithResult(
      const std::string &instanceId, const std::string &nameSpace,
      const std::string &func, std::vector<VALUE_WITH_TYPE *> &params) override;

  void exeJSWithCallback(const std::string &instanceId,
                         const std::string &nameSpace, const std::string &func,
                         std::vector<VALUE_WITH_TYPE *> &params,
                         long callback_id) override;

  int createInstance(const std::string &instanceId, const std::string &func,
                     const std::string &script, const std::string &opts,
                     const std::string &initData, const std::string &extendsApi,
                     std::vector<INIT_FRAMEWORK_PARAMS *> &params) override;

  std::unique_ptr<WeexJSResult> exeJSOnInstance(
      const std::string &instanceId, const std::string &script) override;
 ...
 private:
  void initFrameworkParams(JSContext *ctx,
  ...
  void bindConsoleLog(JSContext *ctx);
